...
namespace Alphora.Dataphor.BOP
{
	...
	public class SettingsList : HashtableList
	{
		...
		public string this[string AKey]
		{
			get 
			{ 
				SettingsItem LItem = (SettingsItem)base[AKey];
				return (LItem == null ? String.Empty : ((SettingsItem)base[AKey]).Value);
			}
			set
			{
				SettingsItem LItem = (SettingsItem)base[AKey];
		...
		{
			base.Add(((SettingsItem)AValue).Name, AValue);
