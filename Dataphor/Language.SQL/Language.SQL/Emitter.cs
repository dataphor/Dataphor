	...
	public class SQLTextEmitter : BasicTextEmitter, ICloneable
	{
		...
		protected override void EmitExpression(Expression expression)
		{
			if (expression is QualifiedFieldExpression)
				...
				EmitSelectStatement(((SelectStatementExpression)expression).SelectStatement);
		...
		{
			EmitExpression(expression.Expression);
			if (!String.IsNullOrEmpty(expression.ColumnAlias))
			{
		...
		{
			if (!String.IsNullOrEmpty(expression.TableSchema))
			{
		...
		{
			Indent();
			AppendFormat("{0} {1} ", Keywords.Alter, Keywords.View);
			if (!String.IsNullOrEmpty(statement.ViewSchema))
			{
				EmitIdentifier(statement.ViewSchema);
				Append(Keywords.Qualifier);
		...
		{
			Indent();
			AppendFormat("{0} {1} ", Keywords.Drop, Keywords.View);
			if (!String.IsNullOrEmpty(statement.ViewSchema))
			{
				EmitIdentifier(statement.ViewSchema);
				Append(Keywords.Qualifier);
