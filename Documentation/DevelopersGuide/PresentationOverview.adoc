...
Frontend.Form(<library name>, <document name>):: This operator allows both static and customized form documents to be
...
Frontend.
Frontend.Image(<library name>, <document name>):: This operator allows documents containing graphic data such as images
and icons to be loaded using streams.
Frontend.Derive(<expression>, <pagetype>, <master key names>, <detail key names>, <elaborate>):: This operator invokes the Frontend Server process of user interface
