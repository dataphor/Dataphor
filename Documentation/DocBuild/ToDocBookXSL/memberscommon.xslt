...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- -->
	<xsl:template match="property[starts-with(@declaringType, 'System.')]" mode="table">
		<xsl:text>&#10;</xsl:text>
		<row>
			<entry colname="col1">
                <para>
	...
	<xsl:template match="method[@declaringType]" mode="table">
		<xsl:variable name="name" select="@name" />
		<xsl:variable name="declaring-type-id" select="concat('T:', @declaringType)" />
		<xsl:if test="not(preceding-sibling::method[@name=$name] and not(child::documentation/nodoc))">
			<xsl:text>&#10;</xsl:text>
			<row>
				<xsl:variable name="declaring-class" select="//class[@id=$declaring-type-id]" />
				<xsl:choose>
	...
	<!-- -->
	<xsl:template match="method[starts-with(@declaringType, 'System.')]" mode="table">
		<xsl:text>&#10;</xsl:text>
		<row>
			<entry colname="col1">
                <para>
	...
	<!-- -->
	<xsl:template match="event[starts-with(@declaringType, 'System.')]" mode="table">
		<xsl:text>&#10;</xsl:text>
		<row>
			<entry colname="col1">
                <para>
    ...
    <xsl:if test="not(child::documentation/nodoc)">
