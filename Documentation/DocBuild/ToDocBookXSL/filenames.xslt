...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="get-filename-for-current-namespace-hierarchy">
    <xsl:value-of select="concat(translate($namespacename, '[,]', ''), 'Hierarchy.html')" />
  </xsl:template>
  <!-- -->
  ...
  <xsl:template name="get-filename-for-current-constructor-overloads">
    ...
    <!-- todo: verify that this fixes the link to the overloaded constructors -->
    <xsl:value-of select="concat($type-part, 'Constructors.html')" />
  </xsl:template>
  <!-- -->
