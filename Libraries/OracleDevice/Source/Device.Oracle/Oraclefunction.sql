/*
	Oracle SQL Implementation Script for the functions that should be mapped to the Oracle Device
*/	
...
/*
	TimeSpan Functions
*/
create or replace function DAE_TSReadMillisecond(ATimeSpan in number)
	return number
	is 
		LReturnVal number(20,0);
	begin
		LReturnVal := TRUNC(DAE_Frac(ATimeSpan / (10000 * 1000)) * 1000);
		return LReturnVal;
	end;
/
...
create or replace function DAE_TSWriteSecond(ATimeSpan in number, APart int)
	...
	is
		LReturnVal number(20,0);
	begin
		LReturnVal := ATimeSpan + (APart - ReadMillisecond(ATimeSpan)) * 10000000;
		return LReturnVal;
	end;
/
create or replace function DAE_TSWriteMinute(ATimeSpan in number, APart int)
	...
	is
		LReturnVal number(20,0);
	begin
		LReturnVal := ATimeSpan + (APart - ReadMillisecond(ATimeSpan)) * 600000000;
		return LReturnVal;
	end;
/
create or replace function DAE_TSWriteHour(ATimeSpan in number, APart int)
	...
	is
		LReturnVal number(20,0);
	begin
		LReturnVal := ATimeSpan + (APart - ReadMillisecond(ATimeSpan)) * 36000000000;
		return LReturnVal;
	end;
/
create or replace function DAE_TSWriteDay(ATimeSpan in number, APart int)
	...
	is
		LReturnVal number(20,0);
	begin
		LReturnVal := ATimeSpan + (APart - ReadMillisecond(ATimeSpan)) * 864000000000;
		return LReturnVal;
	end;
/
