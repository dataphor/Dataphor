...
namespace Alphora.Dataphor.CQF.Model
{
	...
	{
		public static bool ShouldGenerateType(Type type)
		{
			if (type.IsGenericType)
				return false;

			return type.Equals(typeof(Base)) || type.IsSubclassOf(typeof(Base));
		}
		...
		public override object InternalExecute(Program program, object[] arguments)
		{
			...
			// Generate types for all modelinfo classes
			Type[] types = typeof(GenerateTypesNode).Assembly.GetTypes();

			foreach (Type type in types)
			{
				if (ShouldGenerateType(type))
		...
		private void MapPrimitiveTypeNames()
		{
			...
			_typeNameMap.Add("System.Xml.XmlQualifiedName", "String");
		}

		private D4.TypeSpecifier GenerateType(Program program, Type type)
		{
			if (type.IsEnum)
			...
			if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(List<>))
				return new D4.ListTypeSpecifier(GenerateType(program, type.GetGenericArguments()[0]));
